...
<html>
<head>
    <title>Laporan Kriteria Pelatihan Kerja</title>
    <style>
        body {
...
<div class="content">
    <h2>Laporan Kriteria Pelatihan Kerja</h2>
    <table>
        <thead>
            <tr>
                <th>No</th>
        ...
        <tbody>
            <?php
            $rows = $db->get_results("SELECT * FROM tb_dataminat ORDER BY kode_dataminat");
            $no = 0;
            foreach ($rows as $row) : ?>
                <tr>
                    <td><?= ++$no ?></td>
                    <td><?= htmlspecialchars($row->kode_dataminat) ?></td>
                    <td><?= htmlspecialchars($row->nama_dataminat) ?></td>
